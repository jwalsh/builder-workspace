...
def get_project_state(project_name: str) -> Dict:
    ...
    if not project_info_path or not tasks_path:
        click.echo(
            f"Error: Required JSON files not found for project {project_name}", err=True
        )
        return {
            "name": project_name,
...
@click.option(
    ...
    "check_all",
    is_flag=True,
    help="Check all projects, including non-featured ones",
)
def main(project: str, check_all: bool):
    ...
    if project:
        if project not in FEATURED_PROJECTS and not check_all:
            click.echo(
                f"Error: {project} is not a recognized featured project", err=True
            )
            return
    ...
    sorted_states = sorted(
        project_states,
        key=lambda x: (
            x["completed_tasks"] / x["total_tasks"] if x["total_tasks"] > 0 else 0
        ),
        reverse=True,
    )
    ...
    if check_all:
        ...
        overall_progress = (
            f"{(total_completed / total_tasks) * 100:.2f}%"
            if total_tasks > 0
            else "N/A"
        )
