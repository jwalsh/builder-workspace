...
def add_project_version(project_id: str, definition: str):
    try:
        conn = sqlite3.connect(get_db_path())
        c = conn.cursor()
        c.execute('''INSERT OR REPLACE INTO project_versions (id, definition, created_at)
                     VALUES (?, ?, ?)''',
...
def add_task(task: Task):
    try:
        conn = sqlite3.connect(get_db_path())
        c = conn.cursor()
        c.execute('''INSERT INTO tasks (project_id, title, description, status, assigned_to, priority, dependencies, task_type, rfc_state)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                  (task.project_id, task.title, task.description, task.status,
                   task.assigned_to, task.priority, json.dumps(task.dependencies),
                   task.task_type.value, task.rfc_state.value if task.rfc_state else None))
        task_id = c.lastrowid
        conn.commit()
        return task_id
    except sqlite3.Error as e:
    ...
    try:
        ...
        c.execute('''UPDATE tasks SET
                  ...
                  (task.title, task.description, task.status, task.assigned_to,
                   task.priority, json.dumps(task.dependencies), task.task_type.value,
                   task.rfc_state.value if task.rfc_state else None, task.id))
        conn.commit()
    except sqlite3.Error as e:
...
def show_project_tasks(project_name: str):
    try:
        ...
        if not tasks:
            click.echo(f"Warning: No tasks found for project '{project_name}'. The project may not have been decomposed yet.")
            return

        click.echo(f"Tasks for project '{project_name}':")
        for task in tasks:
            click.echo(f"ID: {task[0]}, Priority: {task[1]}, Title: {task[2]}, Status: {task[3]}, Assigned: {task[4]}, Type: {task[5]}, RFC State: {task[6]}")
    except sqlite3.Error as e:
    ...
    try:
        ...
        if not existing_tasks or force:
            ...
            for task in initial_tasks:
                ...
                if task_id:
                    task.id = task_id
                    click.echo(f"Added task: {task.title} (Type: {task.task_type}, Assigned to: {task.assigned_to})")
                else:
        ...
        for rfc_task in rfc_tasks:
            try:
                ...
                click.echo(f"Processed RFC task: {updated_rfc_task.title} (New state: {updated_rfc_task.rfc_state})")
            except Exception as e:
...
@click.option('--use-llm', type=click.Choice(['ollama', 'claude', 'random']), default='ollama', help='Choose LLM provider')
@click.option('--check-health', is_flag=True, help='Check health status of LLM providers')
@click.option('--reset-config', is_flag=True, help='Reset the configuration to default values')
def main(name: str, description: str, force: bool, list: bool, use_llm: str, check_health: bool, reset_config: bool):
    try:
        if reset_config:
            if os.path.exists(CONFIG_FILE):
        ...
        if check_health:
            ...
            click.echo(f"Ollama health: {'Healthy' if config.ollama_healthy else 'Unhealthy'}")
            click.echo(f"Claude health: {'Healthy' if config.claude_healthy else 'Unhealthy'}")
            return
        ...
        if list:
            ...
            if force:
                ...
                if not description:
                    conn = sqlite3.connect(get_db_path())
                    c = conn.cursor()
                    c.execute('SELECT definition FROM project_versions WHERE id = ?', (name,))
                    result = c.fetchone()
                    conn.close()
                    ...
                    if result:
                        try:
                            ...
                            description = existing_project.get('description', 'No description available')
                        except json.JSONDecodeError:
                            click.echo(f"Error: Invalid JSON data for project '{name}'. Please provide a description.")
                            return
                    else:
                        click.echo(f"Error: No existing project found with name '{name}'. Please provide a description.")
                        return
            ...
            click.echo("Please provide a project name or use --list to see all projects.")
    except Exception as e:
