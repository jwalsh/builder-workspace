...
async def test_run_llm_command(mock_llm_manager):
    ...
    mock_llm_manager.run_llm_command.assert_called_once_with("Test prompt", "test_cache_key", "test_role")
...
async def test_llm_manager_run_llm_command(mocker):
    ...
    result = await llm_manager.run_llm_command("Test prompt", "test_cache_key", "test_role")
