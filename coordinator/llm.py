...
class LLMManager:
    ...
    def load_config(self) -> LLMConfig:
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                logging.warning(
                    f"Invalid config file: {e}. Creating a new configuration."
                )
    ...
    async def update_health_status(self, force_check=False):
        if force_check or (datetime.now() - self.config.last_check) > timedelta(
            hours=4
        ):
            self.config.ollama_healthy = await create_llm_provider(
                "ollama"
            ).health_check()
    ...
    async def run_llm_command(
        ...
        if self.config.provider != LLMProvider.OLLAMA:
            providers_to_try.append(
                "ollama"
            )  # Fallback to Ollama if it's not the primary choice
        ...
        for provider_name in providers_to_try:
            ...
            try:
                ...
                result = await provider.generate(
                    prompt, cache_key=cache_key, role=role, **kwargs
                )
                return result
